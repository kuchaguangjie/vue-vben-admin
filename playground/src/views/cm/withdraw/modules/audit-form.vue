...
import { auditUserWithdraw } from '#/api/cm';
...
<template>
  <Drawer :title="$t('cm.userWithdraw.audit')">
    <div v-if="withdrawData" class="mb-4 space-y-2">
      ...
      <p>
        <strong>{{ $t('cm.userWithdraw.feeAmount') }}:</strong>
        {{ withdrawData.feeAmount.toFixed(2) }}
      </p>
      <p>
        <strong>{{ $t('cm.userWithdraw.actualAmount') }}:</strong>
        {{ withdrawData.actualAmount.toFixed(2) }}
      </p>
      <p>
